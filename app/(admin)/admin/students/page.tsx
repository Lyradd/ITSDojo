...
export default function StudentsPage() {
  ...
  // Fetch + polling setiap 5 detik untuk realtime update.
  useEffect(() => {
    if (!isMounted) return;
      ...
      <div className="container mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="mb-8 relative">
          ...
          <div className="relative">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="ml-2 inline-flex items-center gap-1.5 px-2 py-0.5 bg-green-500/10 text-green-600 dark:text-green-400 rounded-full text-[10px] font-black uppercase tracking-widest border border-green-500/20">
                <span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></span>
                Live
              </span>
            </div>
            <p className="text-zinc-600 dark:text-zinc-400 text-lg">
              Pantau progress dan performa semua mahasiswa secara real-time
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <Card className="p-4 rounded-xl border-2 bg-linear-to-br from-purple-500 to-pink-600 text-white">
            ...
            <div className="text-3xl font-bold">{avgXp}</div>
          </Card>
          ...
          <Card className="p-4 rounded-xl border-2 bg-linear-to-br from-orange-500 to-red-600 text-white">
            <div className="text-sm text-orange-100 mb-1">Aktif 24 Jam Terakhir</div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                XP
                {sortBy === 'xp' && <ArrowUpDown className="w-3 h-3 ml-1" />}
              </Button>
              <Button
              ...
              >
                ...
                Akurasi
                {sortBy === 'accuracy' && <ArrowUpDown className="w-3 h-3 ml-1" />}
              </Button>
            </div>
          </div>
        </Card>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-zinc-50 dark:bg-zinc-900 border-b-2">
                <tr>
                  <th className="text-left p-4 font-bold text-sm text-zinc-600 dark:text-zinc-400">Rank</th>
                  <th className="text-left p-4 font-bold text-sm text-zinc-600 dark:text-zinc-400">Mahasiswa</th>
                  <th className="text-left p-4 font-bold text-sm text-zinc-600 dark:text-zinc-400">Semester</th>
                  <th className="text-left p-4 font-bold text-sm text-zinc-600 dark:text-zinc-400">Level</th>
                  <th className="text-left p-4 font-bold text-sm text-zinc-600 dark:text-zinc-400">XP</th>
                  <th className="text-left p-4 font-bold text-sm text-zinc-600 dark:text-zinc-400">Akurasi</th>
                  <th className="text-left p-4 font-bold text-sm text-zinc-600 dark:text-zinc-400">Streak</th>
                  <th className="text-left p-4 font-bold text-sm text-zinc-600 dark:text-zinc-400">Evaluasi</th>
                  <th className="text-left p-4 font-bold text-sm text-zinc-600 dark:text-zinc-400">Terakhir Aktif</th>
                </tr>
              </thead>
              <tbody>
