...
export default function AsdosDashboardPage() {
  ...
  useEffect(() => {
    setIsMounted(true);
    // Redirect if not asdos
    if (isMounted && role !== 'asdos') {
  ...
  if (!isMounted || role !== 'asdos') return null;
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-green-50 via-emerald-50 to-blue-50 dark:from-zinc-950 dark:via-zinc-900 dark:to-zinc-950">
      <div className="container mx-auto max-w-7xl px-4 py-8">
        <div className="mb-8 relative">
          <div className="absolute inset-0 bg-linear-to-r from-green-600 to-blue-600 rounded-3xl opacity-10 blur-3xl"></div>
          <div className="relative">
            ...
            <p className="text-zinc-600 dark:text-zinc-400 text-lg">
              Selamat datang kembali, <span className="font-bold text-green-600">{name}</span> 👋
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {[
            { gradient: 'from-green-500 to-emerald-600', icon: <Users className="w-6 h-6" />, value: stats.totalStudents, label: 'Mahasiswa Terdaftar', textColor: 'text-green-100' },
            { gradient: 'from-blue-500 to-indigo-600', icon: <Activity className="w-6 h-6" />, value: stats.activeToday, label: 'Aktif Hari Ini', textColor: 'text-blue-100' },
            { gradient: 'from-cyan-500 to-blue-600', icon: <Target className="w-6 h-6" />, value: `${stats.averageScore}%`, label: 'Rata-rata Akurasi', textColor: 'text-cyan-100' },
            { gradient: 'from-emerald-500 to-green-600', icon: <ClipboardCheck className="w-6 h-6" />, value: stats.activeEvaluations, label: 'Evaluasi Aktif', textColor: 'text-emerald-100' },
          ].map((stat, i) => (
            <motion.div
              key={i}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
            ...
            <Card className="p-6 rounded-2xl border-2 hover:border-blue-400 transition-all cursor-pointer group">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <div className="font-bold text-zinc-800 dark:text-zinc-100">Lihat Kursus</div>
                  <div className="text-sm text-zinc-600 dark:text-zinc-400">Monitoring materi</div>
                </div>
          ...
          <Link href="/asdos/evaluations">
            <Card className="p-6 rounded-2xl border-2 hover:border-emerald-400 transition-all cursor-pointer group">
              <div className="flex items-center gap-4">
                <div className="p-4 bg-emerald-100 dark:bg-emerald-900/50 rounded-xl group-hover:scale-110 transition-transform">
                  <ClipboardCheck className="w-6 h-6 text-emerald-600 dark:text-emerald-400" />
                </div>
                <div className="flex-1">
                  <div className="font-bold text-zinc-800 dark:text-zinc-100">Monitor Evaluasi</div>
                  <div className="text-sm text-zinc-600 dark:text-zinc-400">Pantau progres kuis</div>
                </div>
                <ArrowRight className="w-5 h-5 text-zinc-400 group-hover:text-emerald-600 transition-all" />
              </div>
            </Card>
          </Link>
